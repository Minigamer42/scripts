/*exported translateShortcodeToUnicode, getShortCodesInMessage, messageContainsShortcodes, getShortCodeMatchingStart*/
"use strict";
...
function translateShortcodeToUnicode (text) {
	...
	getEmojisInMessage(rawText).forEach(emoji => {
		let shortcode = EMOJI_TO_NAME_MAP[emoji];
		let unicodeKey = EMOJI_SHORTCODE_MAP[shortcode].map(entry => entry.toString(16)).join('-');
		codeMap[unicodeKey] = shortcode;
	});
...
function getShortCodesInMessage(msg) {
	let shortcodeRegex = /(:[^:]*(?=:))/g;
	let shortcodesFound = {};
	let match;
	do {
		match = shortcodeRegex.exec(msg);
		if (match && EMOJI_SHORTCODE_MAP[match[1] + ':']) {
...
function getEmojisInMessage(msg) {
	let emojiRegex = /(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g;
	let emojiFound = {};
	let match;
	do {
		match = emojiRegex.exec(msg);
		if (match && EMOJI_TO_NAME_MAP[match[0]]) {
...
function messageContainsShortcodes(msg) {
	return getShortCodesInMessage(msg).length > 0;
}

function getShortCodeMatchingStart(startText) {
	return EMOJI_SHORTCODE_LIST.filter(code => code.startsWith(startText));
}
...
const EMOJI_SHORTCODE_LIST = Object.keys(EMOJI_SHORTCODE_MAP);

const EMOJI_TO_NAME_MAP = Object.keys(EMOJI_SHORTCODE_MAP).reduce((newMap, name) => {
	newMap[String.fromCodePoint(...EMOJI_SHORTCODE_MAP[name])] = name;
	return newMap;
}, {});