...
function ChatBar() {
	...
	this._gameInviteListner = new Listener("game invite", function (payload) {
		this.handleAlert(payload.sender, "Invite to Game #" + payload.gameId, (accept) => {
			if (accept && this.handleActiveGameOnJoin()) {
	...
	this._nexusGameInviteListner = new Listener("nexus game invite", function (payload) {
		this.handleAlert(payload.sender, "Invite to Nexus Game #" + payload.lobbyId, (accept) => {
			if (accept && this.handleActiveGameOnJoin()) {
				socket.sendCommand({
					type: "nexus",
	...
	this._nexusGameInviteListner.bindListener();
	...
	this._chatMessageListener = new Listener("chat message", function (payload) {
		this.handleMessage(payload.sender, payload.message, payload.emojis, payload.modMessage);
		this.toggleIndicators();
	}.bind(this));
...
ChatBar.prototype.handleMessage = function (name, message, emojis, modMessage) {
	this.getChat(name, modMessage).handleMessage(name, message, emojis);
};
...
ChatBar.prototype.handleActiveGameOnJoin = function() {
	if (lobby.inLobby || quiz.inQuiz || nexus.inNexusLobby || nexus.inNexusGame) {
