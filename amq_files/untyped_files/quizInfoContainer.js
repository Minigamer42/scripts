...
function QuizInfoContainer() {
	...
	this.$songInfoLinkRow = $("#qpSongInfoLinkRow");
...
QuizInfoContainer.prototype.displayAnimeName = function (name, altNames = [], altAnswers = []) {
	this.$extraAnimeNameContent = $(
		format(
...
QuizInfoContainer.prototype.displayInfoBoxHint = function ({
	artist,
	songName,
	type,
	typeNumber,
	...
	let diffString = typeof animeDifficulty === "number" ? Math.round(animeDifficulty) : animeDifficulty;
	this.$extraAnimeContent = $(format(this.EXTRA_INFO_TEMPLATE, vintage, animeScore, animeType, diffString, "", ""));
	this.$songName.text(songName);
	this.$songArtist.text(artist);
	this.$songType.text(this.convertTypeToText(type, typeNumber));

	this.$songInfoLinkRow.addClass('hideLinks');
...
QuizInfoContainer.prototype.showAnimeName = function () {
