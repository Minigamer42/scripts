...
class ProfileBadgeOptionContainer {
    constructor(badgeHandler, allBadges, $profileContainer) {
        ...
        let handleBadgeClick = (badgeId) => {
            badgeHandler.optionClick(badgeId);
        };
        ...
        allBadges.sort((a, b) => {
            ...
            if (this.BADGE_TYPE_ORDER_WEIGHT[b.type] !== this.BADGE_TYPE_ORDER_WEIGHT[a.type]) {
    ...
    toggleEdit(on) {
        if (on) {
            ...
            if (!this.badgesLoaded) {
                this.optionSlots.forEach(slot => {
                    slot.load();
                });
                this.chatSlots.forEach(slot => {
                    slot.load();
                });
                this.activeChatSlots.forEach(slot => {
                    slot.load();
                });
    ...
    getNonSpeicalChatBadgeId() {
        let slot = this.activeChatSlots.find((slot) => {
            return !slot.special;
        });
        if (slot) {
            return slot.badgeId;
    ...
    removeChatBadge(badgeId) {
        let index = this.activeChatSlots.findIndex(slot => {
            return slot.badgeId === badgeId;
        });
        if (index !== -1) {
