...
function setup() {
    ...
    new Listener("answer results", (payload) => {
        let songType = payload.songInfo.type - 1;
        if (quiz.teamMode && !quiz.isSpectator && hostModal.$scoring.slider("getValue") === 3) {
            ...
            if (autoTrackCount && Object.keys(answers).length) {
                ...
                if (selfPlayer.correct) {
                    ...
                    if (fastestPlayers.length === 1) {
                        ...
                        if (sonicHeroes) {
                            countButtons[index][songType].addClass("ngmAnimateCorrect");
                            setTimeout(() => {
                                countButtons[index][songType].removeClass("ngmAnimateCorrect");
                            }, 2000);
                        }
                        if (sonicHeroes) {
                            guessCounter[index][songType] -= 1;
                            let sum = 0;
                            for (let i = 0; i < guessCounter.length; i++) {
                                sum += guessCounter[i][songType];
                            }
                            if (sum <= 0) {
                                for (let i = 0; i < guessCounter.length; i++) {
                                    guessCounter[i][songType] = initialGuessCount[i][songType];
                                }
                            }
                        } else if (halfMode) {
                            guessCounter[index] -= 1;
                            if (guessCounter.every((x) => x <= 0)) guessCounter = JSON.parse(JSON.stringify(initialGuessCount));
                        } else if (remainingGuesses === 1) {
                            guessCounter = JSON.parse(JSON.stringify(initialGuessCount));
                        } else {
                            guessCounter[index] -= 1;
                        }
                        countButtons.forEach((element, i) => {
                            if (sonicHeroes) {
                                element[songType].text(guessCounter[i][songType]);
                            } else {
                                element.text(guessCounter[i]);
                            }
                        });
                        if (autoSendTeamCount > 0) {
                            ...
                            if (sonicHeroes) {
                                for (let j = 0; j < 3; j++) {
                                    for (let i = 0; i < guessCounter.length; i++) {
                                        message += guessCounter[i][j];
                                    }
                                    message += ' ';
                                }
                            } else {
                                message = guessCounter.join(halfMode ? " " : "");
                            }
                            sendChatMessage(message, autoSendTeamCount === 1);
                        }
                    } else {
                    ...
                    Object.values(answers).forEach((answer) => {
                        answer.valid = autocomplete.includes(answer.text.toLowerCase());
                    });
                    ...
                    if (validAnswers.length) {
                        ...
                        if (fastestPlayers.length === 1) {
                            ...
                            if (halfModeList[index]) {
                                ...
                                setTimeout(() => {
                                    countButtons[index].removeClass("ngmAnimateWrong");
                                }, 2000);
    ...
    AMQ_addScriptData({
        name: "New Game Mode UI Sonic Heroes",
        author: "Minigamer42 (original by kempanator)",
        description: `
            <p>Version: ${version}</p>
...
function parseMessage(content) {
    if (content === "/version") {
        setTimeout(() => {
            gameChat.systemMessage("New Game Mode UI - " + version);
        }, 1);
    } else if (content === "/ngm") {
        ngmWindow.isVisible() ? ngmWindow.close() : ngmWindow.open();
    }
...
function resetCounter() {
    ...
    if (sonicHeroes) {
        countButtons = Array.from({length: teamList.length}, _ => Array(3).fill(0));
        const songTypes = ['OP', 'ED', 'INS'];
        for (let j = 0; j < 3; j++) {
            $ngmGuessContainer.append(`<span style="width: 32px;display: inline-block;">${songTypes[j]}: </span>`);
            for (let i = 0; i < teamList.length; i++) {
                let $button = $(`<div id="ngmCount_${i}_${j}" class="ngmButton ngmCount"></div>`).click(() => {
                    guessCounter[i][j] <= 0 ? guessCounter[i][j] = initialGuessCount[i][j] : guessCounter[i][j] -= (halfModeList[i][j] ? .5 : 1);
                    countButtons[i][j].text(guessCounter[i][j]);
                });
                countButtons[i][j] = $button;
                $ngmGuessContainer.append($button);
            }
            $ngmGuessContainer.append('<br>');
        }
    } else {
    ...
    if (/^[0-9]+$/.test(countText)) {
        initialGuessCount = Array.from({length: teamList.length}, _ => Array(3));
        if (countText.length === 1) {
            if (sonicHeroes) {
                initialGuessCount = Array.from({length: teamList.length}, _ => Array(3).fill(parseInt(countText)));
            } else {
                initialGuessCount = Array(teamList.length).fill(parseInt(countText));
            }
        } else if (countText.length === teamList.length) {
            if (sonicHeroes) {
                const counts = countText.split("").map((x) => parseInt(x));
                for (let i = 0; i < counts.length; i++) {
                    for (let j = 0; j < 3; j++) {
                        initialGuessCount[i][j] = counts[i];
                    }
                }
            } else {
        ...
        countButtons.forEach((element, i) => {
            if (sonicHeroes) {
                for (let j = 0; j < 3; j++) {
                    element[j].removeClass("disabled").text(guessCounter[i][j]);
                }
            } else {
                element.removeClass("disabled").text(guessCounter[i]);
            }
        });
        let totalGuesses = initialGuessCount.reduce((a, b) => a + b) * (sonicHeroes ? 3 : 1);
        if (totalGuesses === 0) {
            return counterError();
        } else {
...
function setupNGMWindow() {
    ...
    $row5.append($(`<div class="ngmButton" style="width: 34px; background-color: #ffffff; border-color: #cccccc; color: #000000;">SH</div>`)
        .click(function () {
            sonicHeroes = !sonicHeroes;
            if (sonicHeroes) {
                $(this).css({"background-color": "#4497ea", "border-color": "#006ab7", "color": "#ffffff"});
            } else {
                $(this).css({"background-color": "#ffffff", "border-color": "#cccccc", "color": "#333333"});
        ...
        .popover({
            title: "Toggle Sonic Heroes Mode",
            content: "Every player in your team has to guess 1 song from each song type before you can answer the same song type again",
            placement: "bottom",
            trigger: "hover",
            container: "body",
            animation: false,
    ...
    ngmWindow.panels[0].panel.append($row1).append($row2).append($row3).append($row4).append($row5);
}
